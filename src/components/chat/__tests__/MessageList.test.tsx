...
function makeToolMessage(toolName: string, args: Record<string, unknown>, state: "call" | "result" = "result"): Message {
  return {
    id: "1",
    role: "assistant",
    content: "",
...
describe("tool-invocation labels", () => {
  test("str_replace_editor create shows Creating <filename>", () => {
    render(<MessageList messages={[makeToolMessage("str_replace_editor", { command: "create", path: "src/App.jsx" })]} />);
    expect(screen.getByText("Creating App.jsx")).toBeDefined();
  });

  test("str_replace_editor str_replace shows Editing <filename>", () => {
    render(<MessageList messages={[makeToolMessage("str_replace_editor", { command: "str_replace", path: "src/Card.jsx" })]} />);
    expect(screen.getByText("Editing Card.jsx")).toBeDefined();
  });

  test("str_replace_editor insert shows Editing <filename>", () => {
    render(<MessageList messages={[makeToolMessage("str_replace_editor", { command: "insert", path: "src/Card.jsx" })]} />);
    expect(screen.getByText("Editing Card.jsx")).toBeDefined();
  });

  test("file_manager delete shows Deleting <filename>", () => {
    render(<MessageList messages={[makeToolMessage("file_manager", { command: "delete", path: "src/old.jsx" })]} />);
    expect(screen.getByText("Deleting old.jsx")).toBeDefined();
  });

  test("file_manager rename shows Renaming <filename>", () => {
    render(<MessageList messages={[makeToolMessage("file_manager", { command: "rename", path: "src/old.jsx" })]} />);
    expect(screen.getByText("Renaming old.jsx")).toBeDefined();
  });

  test("in-progress tool shows spinner", () => {
    const { container } = render(<MessageList messages={[makeToolMessage("str_replace_editor", { command: "create", path: "src/App.jsx" }, "call")]} />);
    expect(container.querySelector(".animate-spin")).toBeDefined();
  });

  test("completed tool shows no spinner", () => {
    const { container } = render(<MessageList messages={[makeToolMessage("str_replace_editor", { command: "create", path: "src/App.jsx" }, "result")]} />);
