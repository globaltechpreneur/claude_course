...
import { User, Bot, Loader2, FilePlus, FilePen, Trash2, FolderInput } from "lucide-react";
...
function getToolLabel(toolName: string, args: Record<string, unknown>) {
  ...
  if (toolName === "str_replace_editor") {
    switch (args?.command) {
      ...
      case "str_replace":
      case "insert":   return { label: `Editing ${filename}`,   Icon: FilePen };
      default:         return { label: `Reading ${filename}`,   Icon: FilePen };
    }
  }
  if (toolName === "file_manager") {
    if (args?.command === "delete") return { label: `Deleting ${filename}`,  Icon: Trash2 };
    if (args?.command === "rename") return { label: `Renaming ${filename}`,  Icon: FolderInput };
  }
